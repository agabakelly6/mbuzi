...
export function Logo({ tone = "light", className = "" }: LogoProps) {
  ...
  return (
    ...
    >
      {LOGO_SRC ? (
        // Real logo file present (src/assets/branding/logo.*) — renders it
        // in place of the monogram + wordmark below, everywhere Logo is used.
        <img src={LOGO_SRC} alt={`${SITE.name} logo`} className="h-10 w-auto shrink-0" />
      ) : (
        <>
