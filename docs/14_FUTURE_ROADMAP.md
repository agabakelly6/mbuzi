...
- **Per-branch delivery pricing/radius.** Delivery fees are now computed from real routed distance via one shared formula and one shared `MAX_DELIVERY_RADIUS_KM` (`lib/geo.ts`) for every active branch. Extending `Branch.settings` with an optional per-branch rate/radius override (falling back to the shared defaults when absent) is a small, backward-compatible change if a branch ever needs different pricing.
